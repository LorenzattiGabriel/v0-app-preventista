-- Fix: el trigger `trigger_create_order_payment` ya inserta automáticamente
-- una fila en order_payments cuando una orden se crea (no-BORRADOR).
-- Mi RPC también la insertaba → violaba el UNIQUE constraint unique_order_payment(order_id).
-- Solución: usar INSERT ... ON CONFLICT (order_id) DO UPDATE para pisar
-- los valores defecto del trigger con los correctos de la venta.
...
BEGIN
  ...
  INSERT INTO orders (
    ...
    GREATEST(0, v_total - v_account_amount),
    true,
    false,
    v_idempotency_key
  )
  RETURNING id INTO v_order_id;
  -- En este punto el trigger `trigger_create_order_payment` ya insertó una fila
  -- por defecto en order_payments con balance_due = total.
  ...
  -- 3. Cuenta corriente (solo si hay monto a cta cte)
  ...
  -- 4. Pisar order_payments con los valores correctos (el trigger lo creó por defecto)
