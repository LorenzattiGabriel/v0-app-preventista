...
export function ProductForm({ product, initialCode }: ProductFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    category_margin: product?.category_margin?.toString() || "",
    product_margin: product?.product_margin?.toString() || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const productData = {
        ...
        category_margin: parseFloat(formData.category_margin) || 0,
        product_margin: parseFloat(formData.product_margin) || 0,
